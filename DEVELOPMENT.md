...
### v2.16 - linkup.htmlデザイン刷新・トップ画像調整（完了）
**実装日**: 2025-12-11
**説明**: linkup.html（個人配信者の方へ）ページのデザインを刷新。トップページの画像表示を調整。

**変更内容**:

#### 1. linkup.htmlヒーローセクション刷新
**新しいポイント表示**:
- 「マネージャー業務・事務代行をパッケージでご提供」タイトル（白文字）
- 3行のポイント（白背景、左にカラフルライン）:
  - VTuberのマネージャー業務をします！（赤 #dc143c）
  - スケジュール調整、問い合わせ対応などのサポートをします！（オレンジ #ff6b35）
  - 確定申告など、経理業務のサポートをします！（ターコイズ #4ecdc4）
- 「あなたの配信活動を劇的に改善！【10,000円/月～】」フッターテキスト
- 「まずはお問い合わせください」ポップボタン（グラデーションアニメーション）

**黒透明背景ブロック**:
- `.linkup-points`: 黒透明背景（`rgba(0, 0, 0, 0.6)`）
- `backdrop-filter: blur(5px)`
- `border-radius: 10px`

#### 2. サポートセクションのスタイル変更
- `.process-step`背景: 黒透明 → 白チョイ透明（`rgba(255, 255, 255, 0.85)`）
- 説明文（p）: 白文字 → 黒文字（#333333）、フォントサイズ18px

#### 3. トップページ（index.html）画像調整
**問題**: 画面幅1120px以下で画像上部が見切れる

**解決策**:
- `.hero-image-container`: `overflow: hidden`追加、高さ削除
- `.hero-image`: 固定幅1120px（`width: 1120px`, `min-width: 1120px`）
- 画面が小さくなっても画像サイズは変わらず、横は見切れてOK
- `align-items: flex-start`で画像の頭をブラウザ上部に合わせる

**スマホ対応（768px以下）**:
- `min-width: unset`でリセット
- 画面幅に合わせて縮小

#### 4. お問い合わせセクション調整
- `.contact-info p`のmargin-bottom: 0に（プライバシーポリシーリンク下の余白削除）
- `.support-message-box p`: 白文字に変更
